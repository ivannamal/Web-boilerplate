...
/** привести course до канонічного запису або рандом зі списку */
...
/** приблизний підрахунок віку з ISO дати */
...
function fromRandomUser(u) {
    return {
        ...
        // поля під зразок із завдання
        gender: makeFirstLetterCapital(u.gender),                       // "Male"/"Female"
        title: u?.name?.title,
...
/**
 * Створює уніфікований масив користувачів, додає обов’язкові поля,
 * мержить без дублів (randomUserMock + additionalUsers).
 */
export function buildUsers(randomUserMock = [], additionalUsers = []) {
    ...
    // мерж: якщо дубль — зшиваємо, не просто відкидаємо
    const byKey = new Map();
    [...a, ...b].forEach((u) => {
        const k = dedupeKey(u);
        if (!byKey.has(k)) {
...
export function validateUser(user) {
    ...
    const mustCapital = ["full_name","gender","note","state","city","country"];
    for (const k of mustCapital) {
        const v = u[k];
